@import "../TDReader.j"
@import "../TDTokenizer.j"
@import "../TDToken.j"
@import "../TDScientificNumberState.j"

@implementation TDScientificNumberStateTest : OJTestCase
{
    CPString s;
    TDTokenizer t;
    TDReader r;
    TDScientificNumberState sns;
    TDToken tok;
}

- (void)setUp
{
    r = [[TDReader alloc] init];
    t = [[TDTokenizer alloc] init];

    sns = [[TDScientificNumberState alloc] init];
    t.numberState = sns;

    [t setTokenizerState:sns from:'0'.charCodeAt(0) to:'9'.charCodeAt(0)];
    [t setTokenizerState:sns from:'.'.charCodeAt(0) to:'.'.charCodeAt(0)];
    [t setTokenizerState:sns from:'-'.charCodeAt(0) to:'-'.charCodeAt(0)];
}

// - (void)testScientificNumberStringArithmetic
// {
//     t.string = @"1e2 + 1e1 + 1e0 + 1e-1 + 1e-2 + 1e-3";
//     TDArithmeticParser *p = [[[TDArithmeticParser alloc] init] autorelease];
//     var a = [TDTokenAssembly assemblyWithTokenizer:t];
//     var res = [p bestMatchFor:a];
//     tok = [res pop];
//     [self assert:111.111 equals:tok.floatValue];
// }

- (void)testSingleDigit
{
    s = @"3";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:3.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"3" equals:tok.stringValue];
}

- (void)testDoubleDigit
{
    s = @"47";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:47.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"47" equals:tok.stringValue];
}

- (void)testTripleDigit
{
    s = @"654";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:654.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"654" equals:tok.stringValue];
}

- (void)testSingleDigitPositive
{
    s = @"+3";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:3.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+3" equals:tok.stringValue];
}

- (void)testDoubleDigitPositive
{
    s = @"+22";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:22.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
}

- (void)testDoubleDigitPositiveSpace
{
    s = @"+22 ";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:22.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
}

- (void)testMultipleDots
{
    s = @"1.1.1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.1 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"1.1" equals:tok.stringValue];
    
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.1 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@".1" equals:tok.stringValue];
}

- (void)testOneDot
{
    s = @"1.";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"1" equals:tok.stringValue];
}

- (void)testCustomOneDot
{
    s = @"1.";
    [t setString:s];
    [r setString:s];
    sns.allowsTrailingDot = YES;
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"1." equals:tok.stringValue];    
}

- (void)testOneDotZero
{
    s = @"1.0";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"1.0" equals:tok.stringValue];
}

- (void)testPositiveOneDot
{
    s = @"+1.";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+1" equals:tok.stringValue];
}

- (void)testPositiveOneDotCustom
{
    s = @"+1.";
    [t setString:s];
    [r setString:s];
    sns.allowsTrailingDot = YES;
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+1." equals:tok.stringValue];
}

- (void)testPositiveOneDotZero
{
    s = @"+1.0";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+1.0" equals:tok.stringValue];
}

- (void)testPositiveOneDotZeroSpace
{
    s = @"+1.0 ";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+1.0" equals:tok.stringValue];
}

- (void)testNegativeOneDot
{
    s = @"-1.";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-1" equals:tok.stringValue];
}

- (void)testNegativeOneDotCustom
{
    s = @"-1.";
    [t setString:s];
    [r setString:s];
    sns.allowsTrailingDot = YES;
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-1." equals:tok.stringValue];
}

- (void)testNegativeOneDotSpace
{
    s = @"-1. ";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-1" equals:tok.stringValue];
}

- (void)testNegativeOneDotZero
{
    s = @"-1.0";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-1.0" equals:tok.stringValue];
}

- (void)testNegativeOneDotZeroSpace
{
    s = @"-1.0 ";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-1.0" equals:tok.stringValue];
}

- (void)testOneDotOne
{
    s = @"1.1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.1 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"1.1" equals:tok.stringValue];
}

- (void)testZeroDotOne
{
    s = @"0.1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.1 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"0.1" equals:tok.stringValue];
}

- (void)testDotOne
{
    s = @".1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.1 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@".1" equals:tok.stringValue];
}

- (void)testDotZero
{
    s = @".0";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@".0" equals:tok.stringValue];
}

- (void)testNegativeDotZero
{
    s = @"-.0";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-0.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-.0" equals:tok.stringValue];
}

- (void)testPositiveDotZero
{
    s = @"+.0";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+.0" equals:tok.stringValue];
}

- (void)testPositiveDotOne
{
    s = @"+.1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.1 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+.1" equals:tok.stringValue];
}

- (void)testNegativeDotOne
{
    s = @"-.1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-0.1 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-.1" equals:tok.stringValue];
}

- (void)testNegativeDotOneOne
{
    s = @"-.11";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-0.11 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-.11" equals:tok.stringValue];
}

- (void)testNegativeDotOneOneOne
{
    s = @"-.111";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-0.111 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-.111" equals:tok.stringValue];
}

- (void)testNegativeDotOneOneOneZero
{
    s = @"-.1110";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-0.111 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-.1110" equals:tok.stringValue];
}

- (void)testNegativeDotOneOneOneZeroZero
{
    s = @"-.11100";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-0.111 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-.11100" equals:tok.stringValue];
}

- (void)testNegativeDotOneOneOneZeroSpace
{
    s = @"-.1110 ";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-0.111 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-.1110" equals:tok.stringValue];
}

- (void)testZeroDotThreeSixtyFive
{
    s = @"0.365";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.365 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"0.365" equals:tok.stringValue];
}

- (void)testNegativeZeroDotThreeSixtyFive
{
    s = @"-0.365";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-0.365 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-0.365" equals:tok.stringValue];
}

- (void)testNegativeTwentyFourDotThreeSixtyFive
{
    s = @"-24.365";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-24.365 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-24.365" equals:tok.stringValue];
}

- (void)testTwentyFourDotThreeSixtyFive
{
    s = @"24.365";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:24.365 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"24.365" equals:tok.stringValue];
}

- (void)testZero
{
    s = @"0";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"0" equals:tok.stringValue];
}

- (void)testNegativeOne
{
    s = @"-1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-1" equals:tok.stringValue];
}

- (void)testOne
{
    s = @"1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"1" equals:tok.stringValue];
}

- (void)testPositiveOne
{
    s = @"+1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:1.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+1" equals:tok.stringValue];
}

- (void)testPositiveZero
{
    s = @"+0";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+0" equals:tok.stringValue];
}

- (void)testPositiveZeroSpace
{
    s = @"+0 ";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"+0" equals:tok.stringValue];
}

- (void)testNegativeZero
{
    s = @"-0";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:-0.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"-0" equals:tok.stringValue];
}

- (void)testNull
{
    s = @"NULL";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testNil
{
    s = @"nil";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testEmptyString
{
    s = @"";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testDot
{
    s = @".";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testDotSpace
{
    s = @". ";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testDotSpaceOne
{
    s = @". 1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testPlus
{
    s = @"+";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testPlusSpace
{
    s = @"+ ";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testPlusSpaceOne
{
    s = @"+ 1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testMinus
{
    s = @"-";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testMinusSpace
{
    s = @"- ";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testMinusSpaceOne
{
    s = @"- 1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:0.0 equals:tok.floatValue];
    [self assertFalse:tok.isNumber];
}

- (void)testInitSig
{
    s = @"- (id)init {";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:@"-" equals:tok.stringValue];
    [self assert:0.0 equals:tok.floatValue];
}

- (void)test1e1
{
    s = @"1e1";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:10.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
}

- (void)test1e2
{
    s = @"1e2";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:100.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
}

- (void)test2dot0e2
{
    s = @"2.0e2";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:200.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:s equals:tok.stringValue];
}

- (void)test2dot0E2
{
    s = @"2.0E2";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:200.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:s equals:tok.stringValue];
}

- (void)test2e2
{
    s = @"2e2";
    [t setString:s];
    [r setString:s];
    tok = [sns nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    [self assert:200.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:s equals:tok.stringValue];
}

- (void)test2eNegative2Tok
{
    s = @"2e-2";
    [t setString:s];
    [r setString:s];
    
    tok = [t nextToken];
    [self assert:0.02 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:s equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assert:[TDToken EOFToken] same:tok];
}

- (void)test2e2Tok
{
    s = @"2e2";
    [t setString:s];
    [r setString:s];
    
    tok = [t nextToken];
    [self assert:200.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:s equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assert:[TDToken EOFToken] same:tok];
}

- (void)test2e2fooTok
{
    s = @"2e2 foo";
    [t setString:s];
    [r setString:s];
    
    tok = [t nextToken];
    [self assert:200.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"2e2" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assertTrue:tok.isWord];
    [self assert:@"foo" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assert:[TDToken EOFToken] same:tok];
}

- (void)test2eNegative2fooTok
{
    s = @"2e-2 foo";
    [t setString:s];
    [r setString:s];
    
    tok = [t nextToken];
    [self assert:0.02 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"2e-2" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assertTrue:tok.isWord];
    [self assert:@"foo" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assert:[TDToken EOFToken] same:tok];
}

- (void)test2ePositive2fooTok
{
    s = @"2e+2 foo";
    [t setString:s];
    [r setString:s];
    
    tok = [t nextToken];
    [self assert:200.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"2e+2" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assertTrue:tok.isWord];
    [self assert:@"foo" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assert:[TDToken EOFToken] same:tok];
}

- (void)test2dot0ePositive2fooTok
{
    s = @"2.0e+2 foo";
    [t setString:s];
    [r setString:s];
    
    tok = [t nextToken];
    [self assert:200.0 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"2.0e+2" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assertTrue:tok.isWord];
    [self assert:@"foo" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assert:[TDToken EOFToken] same:tok];
}

- (void)test2dot0eNegative2fooTok
{
    s = @"2.0e-2 foo";
    [t setString:s];
    [r setString:s];
    
    tok = [t nextToken];
    [self assert:0.02 equals:tok.floatValue];
    [self assertTrue:tok.isNumber];
    [self assert:@"2.0e-2" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assertTrue:tok.isWord];
    [self assert:@"foo" equals:tok.stringValue];
    
    tok = [t nextToken];
    [self assert:[TDToken EOFToken] same:tok];
}

@end
